    ...
    def add(
        ...
        if lstm_state is not None:
            self.lstm_states.append(
                (
                    lstm_state[0].detach().cpu(),
                    lstm_state[1].detach().cpu(),
                )
            )
        else:
...
def evaluate(
    ...
    with torch.no_grad():
        while any(active):
            ...
            if use_lstm:
                h_cat = torch.cat(
                    [lstm_states[i][0].to(device) for i in active_idx],
                    dim=0,
                )
                c_cat = torch.cat(
                    [lstm_states[i][1].to(device) for i in active_idx],
                    dim=0,
                )
    ...
    # Width-stratified recall accumulators (diagnostic; observation-only).
    rec_thin: List[float] = []
    rec_med: List[float] = []
    ...
    for s_key, cov in per_sample_coverage.items():
        ...
        # Stratify recall by local vessel width if a true vessel mask is
        # available (centerline-as-mask fallback gives uniform widths and
        # the bins are meaningless, so skip in that case).
...
class RunningRewardNormalizer:
    """Scale rewards by their running std — WITHOUT subtracting the mean.

    Mean-centering is deliberately omitted.  Subtracting a running mean
    flips the sign of rewards relative to that mean: ordinary on-vessel
    tracing steps (small positive raw reward) become *negative* once the
    mean is inflated by the large opening coverage steps, and a constant
    terminal penalty (early-stop ≈ −2 on nearly every episode) collapses to
    ≈0 — erasing the very signal that should discourage stopping.  Dividing
    by the running std only (the SB3 ``VecNormalize`` / Engstrom et al. 2020
    "Implementation Matters" convention) keeps positive rewards positive,
    negative penalties negative, and merely stabilises the scale across
    curriculum stages.

    ``running_mean`` is still tracked — it is needed for an unbiased running
    variance (Welford) — but is never subtracted in :meth:`normalize`.
    """
    ...
    def __init__(
        ...
        self.clip = clip
        self.gamma = gamma
        # Cap how much any single raw reward can inflate the running variance.
        # The dense step reward lives in roughly [-0.3, +2.6], so genuine signal
        # is preserved at update_clip=5. Without this, occasional terminal
        # jackpots (e.g. terminal_f1_weight=12 on a strong episode) blow up
        # running_var and shrink the -2 early-stop penalty to noise after
        # normalization. The clip applies only to variance updates; the
        # *original* reward is still what normalize() divides by std.
    ...
    def update(self, reward: float):
        # Clip ONLY the value fed into the running variance, so outlier
        # rewards don't permanently inflate the normalizer's scale.
        r = float(
            np.clip(
                reward,
                -self.update_clip,
...
class PPOTrainer:
    ...
    def __init__(
        ...
        # Burn-in length is read from config (not the trainer's kwargs) so the
        # call sites in scripts/train_ppo.py don't need to change. 0 disables.
        ...
        # Single normalizer for ALL rewards (step + terminal).  The previous
        # split — a separate terminal normalizer — is what destroyed the
        # early-stop signal: terminal rewards are ≈ −2 on almost every
        # episode (early-stop penalty, since single-episode coverage is tiny),
        # so a dedicated mean-centering normalizer collapsed that constant to
        # ≈0 and even made the rarer OOB (−1 > −2 mean) normalize *positive*.
        # With std-only scaling (no mean subtraction, see
        # RunningRewardNormalizer) a single shared normalizer keeps the −2
        # penalty strongly negative and on-vessel steps positive.
    ...
    def _get_curriculum_overrides_dict(
        self,
    ) -> dict:
        ...
        if 'entropy_coef' in train_ov:
            ...
            if ec_end is not None and ec_iters > 0:
                ...
                #
                # Stage B1: bypass the freeze for the first 200 iters of
                # each stage. A stale freeze flag inherited from the
                # previous stage (set when val_clDice plateaued before
                # advancing) was locking new stages at the lowest
                # entropy_coef and preventing the policy from exploring
                # the harder task variants. The freeze is still respected
                # later in the stage when a real plateau is detected.
                ...
                # If frozen and past warmup, leave self.entropy_coef unchanged
            else:
    ...
    def _ppo_update_ff(
        ...
        for _ in range(self.ppo_epochs):
            ...
            for start in range(
                ...
                # The GT class is taken from the center pixel of the junction
                # channel, but if the encoder sees that channel unmodified it
                # can trivially identity-map the label (center pixels are
                # near-uncorrupted by CNN strides), defeating the inductive
                # bias. Mask the entire junction channel on the encoder input
                # so the head must infer junction status from image and other
                # context channels.
                if self.model.junction_head is not None and self._junction_ch_idx is not None:
                    obs_batch = obs[idx]
                    j_vals = obs_batch[
                        :,
                        self._junction_ch_idx,
                        self._obs_center,
                        self._obs_center,
                    ]  # (B,)
                    j_class = torch.zeros(
                ...
                # NaN guard: clip_grad_norm_ does NOT recover NaN/Inf
                # gradients — stepping the optimizer with them would
                # corrupt every parameter irreversibly. Skip the step
                # and zero the grads instead, so training can continue.
    ...
    def _ppo_update_lstm(
        ...
        When ``self.lstm_burn_in > 0``, each chunk additionally extends
        backward by up to ``lstm_burn_in`` steps. Those leading steps run
        through encoder + LSTM (so the recurrent state thaws using real
        observations and the *current* encoder) but their mask is zero —
        no policy/value/junction loss is taken at burn-in positions. The
        training stride remains ``lstm_chunk_length`` so chunks still
        cover the buffer contiguously; only the leading edge overlaps.
        """
        ...
        # Each entry: (buf_idx, actual_s, burn_in_actual, total_len).
        # `actual_s` is where the chunk's tensor begins in the buffer
        # (`logical_s - burn_in_actual`); the first `burn_in_actual` steps
        # are warmup (mask=0). Training stride is T_train so chunks tile
        # the buffer contiguously even when burn-in causes overlap.
        ...
        for buf_idx, buf in enumerate(buffers):
            ...
            for logical_s in range(0, T, T_train):
                ...
                # Need at least 2 *training* steps for the chunk to be useful.
        ...
        # Batch by *training* length so each gradient step still sees roughly
        # mini_batch_size effective transitions. Burn-in compute is overhead
        # not counted against the batch size.
        ...
        for _ in range(self.ppo_epochs):
            ...
            for batch_start in range(0, len(perm), chunks_per_batch):
                ...
                # Per-chunk flag: True iff the chunk starts immediately after
                # an episode boundary (i.e. buf.dones[s-1]==1). For such chunks
                # the stored lstm_state at index s is a *detached* CPU snapshot
                # of init_h/init_c at rollout time — both stale (params have
                # drifted under subsequent PPO updates) and severed from the
                # gradient graph. We substitute the live init params below so
                # chunk-start resets behave identically to mid-chunk resets in
                # forward_sequence and gradients propagate into init_h/init_c.
                ...
                for j, ci in enumerate(batch_idx):
                    ...
                    obs_np[:total_len, j] = np.asarray(
                        buf.obs[s : s + total_len],
                        dtype=np.float32,
                    )
                    actions_np[:total_len, j] = np.asarray(
                        buf.actions[s : s + total_len],
                        dtype=np.int64,
                    )
                    old_lp_np[:total_len, j] = np.asarray(
                        buf.log_probs[s : s + total_len],
                        dtype=np.float32,
                    )
                    ...
                    dones_np[:total_len, j] = np.asarray(
                        buf.dones[s : s + total_len],
                        dtype=np.float32,
                    )
                    # Mask: burn-in prefix is forward-only (mask=0); training
                    # region is mask=1; trailing pad is mask=0.
                    mask_np[
                        burn_in_actual:total_len,
                        j,
                    ] = 1.0
                    ...
                    # Detect chunk-start reset on the *actual* tensor start so
                    # the reset semantics line up with the position the LSTM
                    # init state is loaded into. Buffer-boundary chunks
                    # (actual_s==0) are treated as continuation — we can't tell
                    # from buf.dones alone whether the previous iteration ended
                    # on a done, and one-update of drift there is cheap.
                ...
                # Replace the stored (detached, stale) snapshots for chunks
                # that begin right after a done with the model's *live*
                # learnable init params. Multiplying by the reset mask keeps
                # the gradient path through init_h/init_c only for those
                # rows; non-reset rows still use the rollout-time state.
                if any(reset_flags):
                    reset_b = torch.tensor(
                        reset_flags,
                        dtype=torch.float32,
                        device=self.device,
                    ).unsqueeze(-1)  # (B, 1)
                ...
                # Forward (T, B, ...) once for the main losses. The junction
                # aux head runs on a SEPARATE masked encoder pass (below) so
                # the encoder cannot trivially read the junction-channel
                # center pixel — see _ppo_update_ff for the same fix.
                ...
                # Junction auxiliary loss — separate encoder pass on inputs
                # with the junction channel masked, so the head cannot
                # identity-map the label from the encoder's own input.
                if use_junction_aux:
                    obs_flat = obs_t.reshape(
                        T_chunk * B,
                        *obs_t.shape[2:],
                    )
    ...
    def _collect_rollout_vec(
        ...
        while steps_collected < self.steps_per_iter:
            ...
            if self.use_lstm:
                h_cat = torch.cat(
                    [s[0].to(self.device) for s in lstm_states_list],
                    dim=0,
                )
                h_c_cat = torch.cat(
                    [s[1].to(self.device) for s in lstm_states_list],
                    dim=0,
                )
            ...
            for i in range(n_envs):
                ...
                # Single shared normalizer for step AND terminal rewards.
                # std-only scaling preserves the sign/magnitude of the
                # terminal early-stop penalty (≈ −2) so the policy is
                # actively discouraged from stopping early.
                ...
                if done:
                    ...
                    # Termination-reason instrumentation (Diag-3): tally the
                    # cause so the next training run logs OOB/off_track/STOP/
                    # max_steps fractions and we don't have to back them out
                    # of the per-step r_terminal mean.
                    ...
                    # NEW — record reward-independent behavior signals at episode end
        ...
        with torch.no_grad():
            if self.use_lstm:
                h_cat = torch.cat(
                    [s[0] for s in lstm_states_list],
                    dim=0,
                ).to(self.device)
                h_c_cat = torch.cat(
                    [s[1] for s in lstm_states_list],
                    dim=0,
                ).to(self.device)
    ...
    def train(
        ...
        from environment.vec_env import (
            SubprocVecEnv,
        )
        ...
        _csv_fields = (
            ...
            + [
                ...
                'val_f1',
                'val_cldice',
                # Width-stratified recall (diagnostic, observation-only).
                'val_recall_thin',
                'val_recall_med',
                'val_recall_thick',
                # Termination-reason fractions per iteration (Diag-3 instrumentation)
                'term_stop_frac',
                'term_off_track_frac',
                'term_max_steps_frac',
                'term_oob_frac',
                'on_track_frac',  # mean info["precision"] across episodes
                'mean_cov_at_done',  # mean info["coverage_ratio"] across episodes
                'mean_step_at_stop',  # mean info["step_count"] for STOP episodes only
                'mean_cov_at_stop',  # mean info["coverage_ratio"] for STOP episodes only
            ]
        )
        _csv_file = open(
            log_path,
        ...
        for iteration in range(
            ...
            self._stop_steps: list = []  # info["step_count"] when terminal_reason=="stop"
            self._stop_cov: list = []  # info["coverage_ratio"] when terminal_reason=="stop"
            self.model.eval()
            ...
            # --- Eval ---
            # `did_eval` gates CSV writes for val_* columns this iteration.
            # Python `for` loops don't create scope, so a previously-bound `ev`
            # would otherwise leak stale eval metrics into non-eval rows.
            did_eval = False
            ...
            for _k in RewardCalculator.BREAKDOWN_KEYS:
                _csv_row[_k] = self._rwrd_sums.get(_k, 0.0) / max(
                    self._rwrd_counts.get(_k, 1),
                    1,
                )
            # Termination-reason fractions for this iteration (Diag-3)
            ...
            # NEW — reward-independent behavior counters
                ...
                _csv_row.update(
                    {
                        ...
                        'val_recall_thin': ev.get(
                            'mean_recall_thin',
                            0.0,
                        ),
                        'val_recall_med': ev.get('mean_recall_med', 0.0),
                        'val_recall_thick': ev.get(
                            'mean_recall_thick',
                            0.0,
                        ),
                    }
        ...
        # Render summary plot from the log we just finished writing.  Fully
        # guarded — a plotting failure must never mask a successful run.
        try:
